...
{
	private static bool DoesPointOverlapExistingPoint(PoissonCell[,] cells, Vector2 point, float minRadius, float radius)
	{
	...
	private static void PushPointIntoGrid(PoissonCell[,] cells, PoissonPoint poissonPoint, float minRadius)
	{
		float cellSize = minRadius / Mathf.Sqrt(2);
	...
	private static PoissonCell[,] CreateGrid(Vector2 sampleRegionSize, float minRadius)
	{
		float cellSize = minRadius / Mathf.Sqrt(2);
	...
	private static List<Vector2> CreateRandomPoints(Vector2 sampleRegionSize, float radius, Vector3 offset, int rejectionSamples = 30)
	{
